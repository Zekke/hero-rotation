...
local function Precombat()
  ...
  -- primordial_wave
  if CDsON() and S.PrimordialWave:IsReady() then
...
local function SingleOpen()
  ...
  if CDsON() and S.PrimordialWave:IsReady() and (MaelstromStacks >= 4 and Target:DebuffUp(S.FlameShockDebuff)) then
...
local function Single()
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (S.DoomWinds:CooldownRemains() > 25 or S.DoomWinds:CooldownRemains() <= 5) then
  ...
  if CDsON() and S.PrimordialWave:IsReady() and (Target:DebuffDown(S.FlameShockDebuff)) then
...
local function SingleTotemicOpen()
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (Player:BuffUp(S.LegacyoftheFrostWitchBuff)) then
...
local function SingleTotemic()
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and ((S.DoomWinds:CooldownRemains() > 23 or S.DoomWinds:CooldownRemains() < 7) and (S.PrimordialWave:CooldownRemains() < 20 or Player:BuffUp(S.PrimordialStormBuff) or not S.PrimordialStorm:IsAvailable())) then
    if Cast(S.FeralSpirit, Settings.Enhancement.GCDasOffGCD.FeralSpirit) then return "feral_spirit single_totemic 12"; end
  end
  -- primordial_wave,if=dot.flame_shock.ticking&(raid_event.adds.in>action.primordial_wave.cooldown)|raid_event.adds.in<6
  if CDsON() and S.PrimordialWave:IsReady() and (Target:DebuffUp(S.FlameShockDebuff)) then
...
local function AoeOpen()
  ...
  if CDsON() and S.PrimordialWave:IsReady() and ((MaelstromStacks >= 4) and Target:DebuffUp(S.FlameShockDebuff) and VarFlameShockSaturated) then
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (MaelstromStacks >= 9) then
...
local function Aoe()
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (S.ElementalSpirits:IsAvailable() or S.AlphaWolf:IsAvailable()) then
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (S.DoomWinds:CooldownRemains() > 30 or S.DoomWinds:CooldownRemains() < 7) then
...
local function AoeTotemicOpen()
  ...
  if CDsON() and S.PrimordialWave:IsReady() and (Target:DebuffUp(S.FlameShockDebuff) and VarFlameShockSaturated) then
...
local function AoeTotemic()
  ...
  if CDsON() and S.PrimordialStormAbility:IsCastable() and ((MaelstromStacks >= 10) and (Player:BuffRemains(S.DoomWindsBuff) <= Player:GCD() * 3 or Player:BuffDown(S.DoomWindsBuff) and S.DoomWinds:CooldownRemains() > Player:BuffRemains(S.PrimordialStormBuff) or Player:BuffStack(S.EarthenWeaponBuff) >= 4 or Player:BuffRemains(S.EarthenWeaponBuff) <= Player:GCD() * 3)) then
...
local function Funnel()
  ...
  if CDsON() and S.FeralSpirit:IsCastable() and (S.ElementalSpirits:IsAvailable()) then
  ...
  if CDsON() and S.PrimordialWave:IsReady() and (Player:BuffDown(S.PrimordialWaveBuff)) then
