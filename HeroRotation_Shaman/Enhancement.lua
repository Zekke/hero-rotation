...
local function Aoe()
  ...
  if S.LightningBolt:IsCastable() and (((S.FlameShockDebuff:AuraActiveCount() == EnemiesMeleeCount or S.FlameShockDebuff:AuraActiveCount() >= 6) or (Player:BuffUp(S.PrimordialWaveBuff) and Player:BuffRemains(S.PrimordialWaveBuff) < 5)) and Player:BuffUp(S.PrimordialWaveBuff) and MaelstromStacks == MaxMaelstromStacks and (Player:BuffDown(S.SplinteredElementsBuff) or FightRemains <= 12)) then
