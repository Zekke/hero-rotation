...
local function EvaluateCycleRake(TargetUnit)
  ...
  return (Player:BuffUp(S.SuddenAmbushBuff) and Player:PMultiplier(S.Rake) > Target:PMultiplier(S.Rake) and BTBuffDown(S.Rake))
end
...
local function Variables()
  ...
  -- TEST settings
  VarZerkBiteweave = Settings.Feral.UseBiteweave
end
...
local function AoeBuilder()
  ...
  if S.BrutalSlash:IsReady() then
    ...
    -- TEST : check pourquoi ça évalue tous les ennemis
  end
...
local function Finisher()
  ...
  if S.FerociousBite:IsReady() and (Player:BuffDown(S.ApexPredatorsCravingBuff) and (Player:BuffDown(BsInc) or (Player:BuffUp(BsInc) and not S.SouloftheForest:IsAvailable()))) then
    ...
    elseif Player:Energy() >= 50 then
      ...
      -- TEST
      if Cast(S.FerociousBite, nil, nil, not Target:IsInMeleeRange(8)) then return "ferocious_bite finisher 8"; end
    end
  end
...
local function Berserk()
  ...
  if S.FerociousBite:IsReady() and (ComboPoints == 5 and Target:DebuffRemains(S.RipDebuff) > 8 and VarZerkBiteweave and EnemiesCount11y > 1) then
...
local function Cooldown()
  ...
  -- TODO : incarnation,target_if=max:target.time_to_die,if=(target.time_to_die<fight_remains&target.time_to_die>25)|target.time_to_die=fight_remains
  ...
  -- TODO : berserk,target_if=max:target.time_to_die,if=((target.time_to_die<fight_remains&target.time_to_die>18)|target.time_to_die=fight_remains)&((!variable.lastZerk)|(fight_remains<23)|(variable.lastZerk&!variable.lastConvoke)|(variable.lastConvoke&cooldown.convoke_the_spirits.remains<10))
  ...
  -- TODO : check if this line mandatory
  ...
  -- TODO : convoke_the_spirits,target_if=max:target.time_to_die,if=((target.time_to_die<fight_remains&target.time_to_die>5)|target.time_to_die=fight_remains)&(fight_remains<5|(dot.rip.remains>5&buff.tigers_fury.up&(combo_points<2|(buff.bs_inc.up&combo_points=2))&(!variable.lastConvoke|!variable.lastZerk|buff.bs_inc.up)))
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.FerociousBite:IsReady() and (Player:BuffUp(S.ApexPredatorsCravingBuff) and (EnemiesCount11y == 1 or (not S.PrimalWrath:IsAvailable()) or Player:BuffDown(S.SabertoothBuff)) and not (VarNeedBT and CountActiveBtTriggers() == 2)) then
      ...
      -- TEST
      if Cast(S.FerociousBite, nil, nil, not Target:IsInMeleeRange(8)) then return "ferocious_bite main 16"; end
    end
