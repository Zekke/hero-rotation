...
local function RotationBOC()
  ...
  if CDsON() and S.WeaponsofOrder:IsCastable() and (RecentPurifiesBuff and S.ImprovedInvokeNiuzao:IsAvailable()) then
    if Cast(S.WeaponsofOrder, nil, Settings.Commons.DisplayStyle.Signature) then return "weapons_of_order rotation_boc 6"; end
  end
  ...
  if CDsON() and S.WeaponsofOrder:IsCastable() and (not S.ImprovedInvokeNiuzao:IsAvailable()) then
    if Cast(S.WeaponsofOrder, nil, Settings.Commons.DisplayStyle.Signature) then return "weapons_of_order rotation_boc 14"; end
  end
