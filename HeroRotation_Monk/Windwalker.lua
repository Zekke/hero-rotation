...
local function Precombat()
  ...
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (not S.JadefireStomp:IsAvailable()) then
...
local function Trinkets()
  ...
  if VarTrinketType == 1 then
    ...
    if Settings.Commons.Enabled.Trinkets then
      ...
      if I.IrideusFragment:IsEquippedAndReady() and (not Trinket1:HasUseBuff() and not Trinket2:HasUseBuff() or (Trinket1:HasUseBuff() or Trinket2:HasUseBuff()) and (Player:BuffUp(S.InvokersDelightBuff) or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 45 and FightRemains > 180) or BossFightRemains < 20) then
...
local function Opener()
  ...
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:Chi() > 1 and Player:ChiDeficit() >= 2) then
...
local function CDSEF()
  ...
  if S.TouchofDeath:CooldownUp() then
    ...
    if ToDTar then
      ...
      if VarDungeonRoute and ComboStrike(S.TouchofDeath) and ToDTar:Health() < Player:Health() or Player:BuffRemains(S.HiddenMastersForbiddenTouchBuff) < 2 or Player:BuffRemains(S.HiddenMastersForbiddenTouchBuff) > ToDTar:TimeToDie() then
...
local function DefaultAoE()
  ...
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:Chi() < 5 and (Player:BloodlustUp() or Player:Energy() < 50)) then
  ...
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2) then
...
local function Default3T()
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and S.RisingSunKick:CooldownDown() and S.FistsofFury:CooldownDown() and Player:Chi() > 4 and ((S.StormEarthAndFire:IsAvailable() and not S.BonedustBrew:IsAvailable()))) then
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and (ComboStrike(S.SpinningCraneKick) and (Player:Chi() > 5 and S.StormEarthAndFire:IsAvailable()))) then
...
local function Default2T()
  ...
  -- chi_burst,if=buff.bloodlust.up&chi<5
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:BloodlustUp() and Player:Chi() < 5) then
  ...
  -- chi_burst,if=chi<5&energy<50
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:Chi() < 5 and Player:Energy() < 50) then
...
local function DefaultST()
  ...
  if S.StrikeoftheWindlord:IsReady() and (BossFightRemains < 5 or S.Thunderfist:IsAvailable() and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 10 and Player:BuffDown(S.DomineeringArroganceBuff) or S.Thunderfist:IsAvailable() and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 35) then
...
local function Fallthru()
  ...
  if S.CracklingJadeLightning:IsReady() and (Player:BuffStack(S.TheEmperorsCapacitorBuff) > 19 and EnergyTimeToMaxRounded() > S.CracklingJadeLightning:ExecuteTime() - 1 and S.RisingSunKick:CooldownRemains() > S.CracklingJadeLightning:ExecuteTime() or Player:BuffStack(S.TheEmperorsCapacitorBuff) > 14 and BossFightRemains < 5) then
  ...
  if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2 and EnemiesCount8y >= 2) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if Settings.Commons.Enabled.Potions then
      ...
      if PotionSelected and PotionSelected:IsReady() then
        if S.InvokeXuenTheWhiteTiger:IsAvailable() then
          ...
          if Player:BuffUp(S.StormEarthAndFireBuff) and XuenActive or BossFightRemains <= 30 then
    ...
    if (HL.CombatTime() < 4 and Player:Chi() < 5 and (not XuenActive or not S.InvokeXuenTheWhiteTiger:IsAvailable())) then
    ...
    if S.TigerPalm:IsReady() and (Player:Energy() > 50 and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3 and ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= (2 + num(Player:BuffUp(S.PowerStrikesBuff))) and (not S.InvokeXuenTheWhiteTiger:IsAvailable() or ((not S.Skyreach:IsAvailable() and not S.Skytouch:IsAvailable()) or HL.CombatTime() > 5 or XuenActive)) and not VarHoldTPRSK and (EnemiesCount8y > 1 or not S.BonedustBrew:IsAvailable() or S.BonedustBrew:IsAvailable() and EnemiesCount8y == 1 and S.BonedustBrew:CooldownDown())) then
    ...
    if S.TigerPalm:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3 and ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= (2 + num(Player:BuffUp(S.PowerStrikesBuff))) and (not S.InvokeXuenTheWhiteTiger:IsAvailable() or ((not S.Skyreach:IsAvailable() and not S.Skytouch:IsAvailable()) or HL.CombatTime() > 5 or XuenActive)) and not VarHoldTPRSK and (EnemiesCount8y > 1 or not S.BonedustBrew:IsAvailable() or S.BonedustBrew:IsAvailable() and EnemiesCount8y == 1 and S.BonedustBrew:CooldownDown())) then
    ...
    if S.ChiBurst:IsCastable() and Player:BuffUp(S.ChiBurstBuff) and (S.JadefireStomp:IsAvailable() and S.JadefireStomp:CooldownDown() and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2 and EnemiesCount8y >= 2) and not S.JadefireHarmony:IsAvailable()) then
