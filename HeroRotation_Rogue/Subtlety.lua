...
local function Finish (ReturnSpellOnly, ForceStealth)
  ...
  if S.Rupture:IsCastable() then
    ...
    and (S.Flagellation:CooldownRemains() >= 10 or Settings.Subtlety.DontHoldForCDs or not CDsON()) then
      if ReturnSpellOnly then
        return S.Rupture
      else
  ...
  if S.Eviscerate:IsCastable() then
    if (S.Flagellation:CooldownRemains() >= 10 or not CDsON()) or (MeleeEnemies10yCount >= 3 and not Target:NPCID() == 241800) or (MeleeEnemies10yCount >= 3 and Target:NPCID() == 241800 and S.Flagellation:CooldownRemains() >= 8) or Settings.Subtlety.DontHoldForCDs then
      if ReturnSpellOnly then
        return S.Eviscerate
      else
