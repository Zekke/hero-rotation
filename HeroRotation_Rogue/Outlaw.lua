...
local function CDs ()
  ...
  if CDsON() and S.KeepItRolling:IsCastable() and Cache.APLVar.RtB_Buffs.Total >= 4
  ...
  if CDsON() and S.KeepItRolling:IsCastable() and Cache.APLVar.RtB_Buffs.Total >= 3 and (num(Player:BuffUp(S.Broadside)) + num(Player:BuffUp(S.RuthlessPrecision)) + num(Player:BuffUp(S.TrueBearing)) >= 2)
...
local function APL ()
  ...
  if not Player:AffectingCombat() and S.Vanish:TimeSinceLastCast() > 1 then
    ...
    if Everyone.TargetIsValid() then
      ...
      if CDsON() and S.AdrenalineRush:IsCastable() and S.ImprovedAdrenalineRush:IsAvailable() and S.KeepItRolling:IsAvailable()
      ...
      if CDsON() and S.AdrenalineRush:IsCastable() and S.ImprovedAdrenalineRush:IsAvailable() then
        if Cast(S.AdrenalineRush, Settings.Outlaw.OffGCDasOffGCD.AdrenalineRush) then
