...
local function Finishers()
  ...
  if S.HammerofLight:IsReady() and (Player:BuffUp(S.HammerofLightBuff) or not S.DivineHammer:IsAvailable() or Paladin.DivineHammerActive or S.DivineHammer:CooldownRemains() > 10 or Settings.Retribution.DisableDivineHammerCheck or not CDsON()) then
  ...
  if VerdictSpell:IsReady() and ((Settings.Retribution.DisableCrusadeAWCDCheck or not S.Crusade:IsAvailable() or S.Crusade:CooldownRemains() > PlayerGCD * 3 or Player:BuffUp(S.CrusadeBuff) and Player:BuffStack(S.CrusadeBuff) < 10 or S.RadiantGlory:IsAvailable()) and Player:BuffDown(S.HammerofLightBuff) and (Settings.Retribution.DisableDivineHammerCheck or S.DivineHammer:CooldownDown() or Paladin.DivineHammerActive or not S.DivineHammer:IsAvailable() or not CDsON())) then
