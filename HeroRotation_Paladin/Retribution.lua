...
local function Cooldowns()
  ...
  if CDsON() and S.AvengingWrath:IsCastable() and ((HolyPower >= 4 and HL.CombatTime() < 5 or HolyPower >= 3 and HL.CombatTime() > 5 or HolyPower >= 2 and S.DivineAuxiliary:IsAvailable() and (S.ExecutionSentence:CooldownUp() or S.FinalReckoning:CooldownUp())) and (EnemiesCount8y == 1 or Target:TimeToDie() > 10)) then
  ...
  if CDsON() and S.Crusade:IsCastable() and (HolyPower >= 5 and HL.CombatTime() < 5 or HolyPower >= 3 and HL.CombatTime() >= 5) then
  ...
  if CDsON() and S.FinalReckoning:IsCastable() and (
    (
      (HolyPower >= 4 and HL.CombatTime() < 8)
      or (HolyPower >= 3 and HL.CombatTime() >= 8)
      or (HolyPower >= 2 and (S.DivineAuxiliary:IsAvailable() or S.RadiantGlory:IsAvailable()))
    )
    and (
    Settings.Retribution.DisableCrusadeAWCDCheck
    or S.AvengingWrath:CooldownRemains() > 10
    or (S.Crusade:CooldownDown() and (Player:BuffDown(S.CrusadeBuff) or Player:BuffStack(S.CrusadeBuff) >= 10))
    or (S.RadiantGlory:IsAvailable() and (S.WakeofAshes:CooldownRemains() < PlayerGCD))
    )
  )
...
local function Finishers()
  ...
  if S.HammerofLight:IsReady() and (S.HammerofLight:IsReady() or not S.DivineHammer:IsAvailable() or Paladin.DivineHammerActive or S.DivineHammer:CooldownRemains() > 10 or not CDsON()) then
  ...
  if S.DivineStorm:IsReady() and (VarDSCastable and not S.HammerofLight:IsReady() and (S.DivineHammer:CooldownDown() or Paladin.DivineHammerActive or not S.DivineHammer:IsAvailable() or not CDsON()) and (Settings.Retribution.DisableCrusadeAWCDCheck or not S.Crusade:IsAvailable() or S.Crusade:CooldownRemains() > PlayerGCD * 3 or Player:BuffUp(S.CrusadeBuff) and Player:BuffStack(S.CrusadeBuff) < 10 or S.RadiantGlory:IsAvailable())) then
  ...
  if VerdictSpell:IsReady() and ((Settings.Retribution.DisableCrusadeAWCDCheck or not S.Crusade:IsAvailable() or S.Crusade:CooldownRemains() > PlayerGCD * 3 or Player:BuffUp(S.CrusadeBuff) and Player:BuffStack(S.CrusadeBuff) < 10 or S.RadiantGlory:IsAvailable()) and not S.HammerofLight:IsReady() and (S.DivineHammer:CooldownDown() or Paladin.DivineHammerActive or not S.DivineHammer:IsAvailable() or not CDsON())) then
...
local function Generators()
  ...
  if CDsON() and S.DivineToll:IsCastable() and (HolyPower <= 2 and (Settings.Retribution.DisableCrusadeAWCDCheck or S.AvengingWrath:CooldownRemains() > 15 or S.Crusade:CooldownRemains() > 15 or S.RadiantGlory:IsAvailable() or BossFightRemains < 8)) then
